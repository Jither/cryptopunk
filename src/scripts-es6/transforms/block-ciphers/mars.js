...
import { rol, mul } from "../../cryptopunk.bitarith";
...
const SBOX1 = SBOX.subarray(256);
const B = SBOX.subarray(265, 269);

function E(a, k1, k2)
{
	let M = a + k1;
	let R = mul(rol(a, 13), k2);
	const i = M & 0b111111111;
	let L = SBOX[i];
	R = rol(R, 5);
	let r = R & 0b11111;
	M = rol(M, r);
...
class MarsTransform extends BlockCipherTransform
{
	constructor(decrypt)
	{
		super(decrypt);
		this.addOption("variant", "Variant", "tweak", { type: "select", texts: VARIANT_NAMES, values: VARIANT_VALUES });
	}

	transform(bytes, keyBytes)
	{
		this.checkBytesSize("Key", keyBytes, { min: 128, max: 448 });

		const keys = this.options.variant === "tweak" ? this.generateKeysTweak(keyBytes) : this.generateKeys(keyBytes);
	...
	generateKeys(keyBytes)
	{
		...
		for (let i = 0; i < 39; i++)
		{
			t[i + 7] = rol(t[i] ^ t[i + 5], 3) ^ keyWords[i % keyWordsLength] ^ i;
		}
		...
		for (let iteration = 0; iteration < 7; iteration++)
		{
			...
			{
				const next = (i + 1) % 40;
				t[next] = rol(t[next] + SBOX[t[i] & 0b111111111], 9);
			}
		}

		const keys = new Uint32Array(40);
		for (let i = 0; i < 40; i++)
		{
			keys[i * 7 % 40] = t[i];
		}

		return this.fixMultiplierKeys(keys, 3, 0xfffffffc);
	}

	generateKeysTweak(keyBytes)
	{
		const temp = new Uint32Array(15);
		// Set temp array to start with key converted to (little endian) (d)words:
		const keyWords = bytesToInt32sLE(keyBytes);
		temp.set(keyWords);
		// Set the next (d)word to the number of key d(words)
		temp[keyWords.length] = keyWords.length;
		...
		for (let iteration = 0; iteration < 4; iteration++)
		{
			...
			{
				temp[i] ^= rol(temp[mod(i - 7, 15)] ^ temp[mod(i - 2, 15)], 3) ^ (4 * i + iteration);
			}
			...
			for (let r = 0; r < 4; r++)
			{
				for (let i = 0; i < temp.length; i++)
				{
					temp[i] = rol(temp[i] + SBOX[temp[mod(i - 1, 15)] & 0b111111111], 9);
				}
			}

			for (let i = 0; i < 10; i++)
			{
				keys[10 * iteration + i] = temp[mod(4 * i, 15)];
			}
		}

		return this.fixMultiplierKeys(keys, -1, 0x7ffffffc);
	}

	fixMultiplierKeys(keys, offset, wordMask)
	{
		for (let i = 5; i <= 35; i += 2)
		{
			...
			// Find consecutive 0's or 1's in key
			const mask = this.getConsecutiveBitMask(w, wordMask);
			if (mask)
			{
				const r = keys[i + offset] & 0b11111;
				const p = rol(B[j], r);
	...
	getConsecutiveBitMask(x, wordMask)
	{
		let m = 0;
		let mask = 0x3ff;
		let j = 0;
		...
		const xn = ~x;
		m &= wordMask & ((x << 1) ^ xn) & ((x >>> 1) ^ xn);

		return m >>> 0;
	}
}

class MarsEncryptTransform extends MarsTransform
{
	constructor()
	{
		super(false);
	...
	transformBlock(block, dest, destOffset, keys)
	{
		...
		for (let i = 0; i < 16; i++)
		{
			const [l, m, r] = E(a, keys[2 * i + 4], keys[2 * i + 5]);
			a = rol(a, 13);
			c += m;
			if (i < 8)
			{
...
class MarsDecryptTransform extends MarsTransform
{
	constructor()
	{
		super(true);
...
export {
	MarsEncryptTransform,
	MarsDecryptTransform
};