import { BlockCipherTransform } from "./block-cipher";
import { int64sToBytesBE, bytesToInt64sBE } from "../../cryptopunk.utils";
import { shl64, shr64, xor64, ZERO_64 } from "../../cryptopunk.bitarith";

const ROUNDS = 6;
const ROUND_KEYS = ROUNDS + 1;

// TODO: SHARK Affine

// Note: S-Boxes are same as SQUARE
const SBOX_ENC = [
	
];

const SBOX_DEC = [
	
];

const G = [
	0xce, 0x95, 0x57, 0x82, 0x8a, 0x19, 0xb0, 0x01, 
	0xe7, 0xfe, 0x05, 0xd2, 0x52, 0xc1, 0x88, 0xf1, 
	0xb9, 0xda, 0x4d, 0xd1, 0x9e, 0x17, 0x83, 0x86, 
	0xd0, 0x9d, 0x26, 0x2c, 0x5d, 0x9f, 0x6d, 0x75, 
	0x52, 0xa9, 0x07, 0x6c, 0xb9, 0x8f, 0x70, 0x17, 
	0x87, 0x28, 0x3a, 0x5a, 0xf4, 0x33, 0x0b, 0x6c, 
	0x74, 0x51, 0x15, 0xcf, 0x09, 0xa4, 0x62, 0x09, 
	0x0b, 0x31, 0x7f, 0x86, 0xbe, 0x05, 0x83, 0x34
];

const G_INV = [
	0xe7, 0x30, 0x90, 0x85, 0xd0, 0x4b, 0x91, 0x41, 
	0x53, 0x95, 0x9b, 0xa5, 0x96, 0xbc, 0xa1, 0x68, 
	0x02, 0x45, 0xf7, 0x65, 0x5c, 0x1f, 0xb6, 0x52, 
	0xa2, 0xca, 0x22, 0x94, 0x44, 0x63, 0x2a, 0xa2, 
	0xfc, 0x67, 0x8e, 0x10, 0x29, 0x75, 0x85, 0x71, 
	0x24, 0x45, 0xa2, 0xcf, 0x2f, 0x22, 0xc1, 0x0e, 
	0xa1, 0xf1, 0x71, 0x40, 0x91, 0x27, 0x18, 0xa5, 
	0x56, 0xf4, 0xaf, 0x32, 0xd2, 0xa4, 0xdc, 0x71, 
];

let CBOX_ENC, CBOX_DEC;
let LOG, ALOG;

function precomputeCboxes(cboxes, sbox, g)
{
	for (let i = 0; i < 8; i++)
	{
		const cbox = cboxes[i] = new Array(512);
		for (let j = 0; j < 256; j++)
		{
			let cboxHi = 0;
			let cboxLo = 0;
			const sboxValue = sbox[j];
			if (sboxValue)
			{
				for (let k = 0; k < 4; k++)
				{
					cboxHi <<= 8;
					cboxHi |= gfMultiply(sboxValue, g[k * 8 + i]);
				}
				for (let k = 4; k < 8; k++)
				{
					cboxLo <<= 8;
					cboxLo |= gfMultiply(sboxValue, g[k * 8 + i]);
				}
			}
			cbox[j * 2] = cboxHi;
			cbox[j * 2 + 1] = cboxLo;
		}
	}
}

// Same as SQUARE (except OFFSETS not used)
function precompute()
{
	if (LOG)
	{
		return;
	}

	LOG = new Array(256);
	ALOG = new Array(256);

	LOG[0] = 0;
	let alog = ALOG[0] = 1;
	for (let i = 1; i < 256; i++)
	{
		alog <<= 1;
		if (alog & 0x100)
		{
			alog ^= 0x1f5;
		}
		ALOG[i] = alog;
		LOG[alog] = i < 255 ? i : 0;
	}

	CBOX_ENC = new Array(8);
	CBOX_DEC = new Array(8);

	precomputeCboxes(CBOX_ENC, SBOX_ENC, G);
	precomputeCboxes(CBOX_DEC, SBOX_DEC, G_INV);
}

function gfMultiply(a, b)
{
	if (a === 0 || b === 0)
	{
		return 0;
	}
	return ALOG[(LOG[a] + LOG[b]) % 255];
}

function transform(a)
{
	const
		k = new Array(8),
		t = new Array(8);

	for (let i = 0; i < 8; i++)
	{
		k[i] = shr64(a, 56 - 8 * i).lo & 0xff;
	}

	for (let i = 0; i < 8; i++)
	{
		t[i] = 0;
		for (let j = 0; j < 8; j++)
		{
			t[i] ^= gfMultiply(G_INV[i * 8 + j], k[j]);
		}
	}

	let r = { hi: 0, lo: 0 };
	for (let i = 0; i < 8; i++)
	{
		r = shl64(r, 8);
		r.lo ^= t[i];
	}

	return r;
}

function crypt(text, keys, sbox, cboxes)
{
	let temp = { hi: text.hi, lo: text.lo };

	for (let r = 0; r < ROUNDS - 1; r++)
	{
		temp = xor64(temp, keys[r]);
		const
			index0 =  temp.hi >>> 24,
			index1 = (temp.hi >>  16) & 0xff,
			index2 = (temp.hi >>   8) & 0xff,
			index3 = (temp.hi       ) & 0xff,
			index4 =  temp.lo >>> 24,
			index5 = (temp.lo >>  16) & 0xff,
			index6 = (temp.lo >>   8) & 0xff,
			index7 = (temp.lo       ) & 0xff;

		temp = xor64(
				{ hi: cboxes[0][index0 * 2], lo: cboxes[0][index0 * 2 + 1] },
				{ hi: cboxes[1][index1 * 2], lo: cboxes[1][index1 * 2 + 1] },
				{ hi: cboxes[2][index2 * 2], lo: cboxes[2][index2 * 2 + 1] },
				{ hi: cboxes[3][index3 * 2], lo: cboxes[3][index3 * 2 + 1] },
				{ hi: cboxes[4][index4 * 2], lo: cboxes[4][index4 * 2 + 1] },
				{ hi: cboxes[5][index5 * 2], lo: cboxes[5][index5 * 2 + 1] },
				{ hi: cboxes[6][index6 * 2], lo: cboxes[6][index6 * 2 + 1] },
				{ hi: cboxes[7][index7 * 2], lo: cboxes[7][index7 * 2 + 1] }
		);
	}

	temp = xor64(temp, keys[ROUNDS - 1]);

	temp = {
		hi: (sbox[(temp.hi >>> 24)       ] << 24) ^
			(sbox[(temp.hi >>  16) & 0xff] << 16) ^
			(sbox[(temp.hi >>   8) & 0xff] <<  8) ^
			(sbox[(temp.hi       ) & 0xff]      ),
		lo: (sbox[(temp.lo >>> 24) & 0xff] << 24) ^
			(sbox[(temp.lo >>  16) & 0xff] << 16) ^
			(sbox[(temp.lo >>   8) & 0xff] <<  8) ^
			(sbox[(temp.lo       ) & 0xff]      )
	};
	return xor64(temp, keys[ROUNDS]);
}

class SharkTransform extends BlockCipherTransform
{
	constructor(decrypt)
	{
		super(decrypt);
	}

	generateSubKeys(keyBytes)
	{
		const a = new Array(ROUND_KEYS);

		let i = 0;
		for (let r = 0; r < ROUND_KEYS; r++)
		{
			a[r] = {
				hi: (keyBytes[(i++) % 16] << 24) |
					(keyBytes[(i++) % 16] << 16) |
					(keyBytes[(i++) % 16] <<  8) |
					(keyBytes[(i++) % 16]),
				lo: (keyBytes[(i++) % 16] << 24) |
					(keyBytes[(i++) % 16] << 16) |
					(keyBytes[(i++) % 16] <<  8) |
					(keyBytes[(i++) % 16])
			};
		}

		const tempKeys = new Array(ROUND_KEYS);
		for (let r = 0; r < ROUND_KEYS; r++)
		{
			tempKeys[r] = { hi: CBOX_ENC[0][r * 2], lo: CBOX_ENC[0][r * 2 + 1] };
		}
		tempKeys[ROUNDS] = transform(tempKeys[ROUNDS]);

		let result = new Array(ROUND_KEYS);

		result[0] = xor64(a[0], crypt(ZERO_64, tempKeys, SBOX_ENC, CBOX_ENC));
		for (let r = 1; r < ROUND_KEYS; r++)
		{
			result[r] = xor64(a[r], crypt(result[r - 1], tempKeys, SBOX_ENC, CBOX_ENC));
		}
		result[ROUNDS] = transform(result[ROUNDS]);

		if (this.decrypt)
		{
			const decryptResult = new Array(ROUND_KEYS);
			decryptResult[0] = result[ROUNDS];
			decryptResult[ROUNDS] = result[0];
			for (let r = 1; r < ROUNDS; r++)
			{
				decryptResult[r] = transform(result[ROUNDS - r]);
			}

			result = decryptResult;
		}

		return result;
	}

	transform(bytes, keyBytes)
	{
		this.checkKeySize(keyBytes, 128);

		precompute();

		const subKeys = this.generateSubKeys(keyBytes);

		return this.transformBlocks(bytes, 64, subKeys);
	}
}

class SharkEncryptTransform extends SharkTransform
{
	constructor()
	{
		super(false);
	}

	transformBlock(block, dest, destOffset, subKeys)
	{
		const text = bytesToInt64sBE(block)[0];
		const result = crypt(text, subKeys, SBOX_ENC, CBOX_ENC);
		dest.set(int64sToBytesBE([result]), destOffset);
	}
}

class SharkDecryptTransform extends SharkTransform
{
	constructor()
	{
		super(true);
	}


	transformBlock(block, dest, destOffset, subKeys)
	{
		const text = bytesToInt64sBE(block)[0];
		const result = crypt(text, subKeys, SBOX_DEC, CBOX_DEC);
		dest.set(int64sToBytesBE([result]), destOffset);
	}
}

export {
	SharkEncryptTransform,
	SharkDecryptTransform
};