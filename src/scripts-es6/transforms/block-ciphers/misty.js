...
function FO(x, ko, ki)
{
	...
	return ((xl ^ ko[3]) << 16) | xr;
}

class MistyBaseTransform extends BlockCipherTransform
{
	constructor(decrypt)
	{
		super(decrypt);
	...
	transformBlock(block, dest, destOffset, keys)
	{
		...
		for (let i = 0; i < rounds; i++)
		{
			if (i % 2 === 0)
			{
				// Odd rounds (MISTY counts from 1)
				l = this.FL(l, KL[kl++]);
				r = this.FL(r, KL[kl++]);
			}
	...
	arrangeRoundKeys(keys)
	{
		...
		const oddRounds = rounds / 2 + 1;
		for (let i = 0; i < oddRounds; i ++)
		{
			kl.push(
				[ userKeys[i % 8], derivedKeys[(i + 6) % 8] ],
				[ derivedKeys[(i + 2) % 8], userKeys[(i + 4) % 8] ]
			);
		}
	...
	generateSubKeys(keyBytes)
	{
		...
		for (let i = 0; i < 8; i++)
		{
			derivedKeys[i] = FI(userKeys[i], userKeys[(i + 1) % 8]);
		}
...
class MistyEncryptTransform extends MistyBaseTransform
{
	constructor()
	{
		super(false);
...
class MistyDecryptTransform extends MistyBaseTransform
{
	constructor()
	{
		super(true);
...
export {
	MistyEncryptTransform,
	MistyDecryptTransform
};