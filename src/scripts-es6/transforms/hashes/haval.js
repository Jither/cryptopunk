...
class HavalTransform extends HashTransform
{
	...
	transformBlock(block, state, options)
	{
		...
		state[0] = add(state[0], t0);
		state[1] = add(state[1], t1);
		state[2] = add(state[2], t2);
		state[3] = add(state[3], t3);
		state[4] = add(state[4], t4);
		state[5] = add(state[5], t5);
		state[6] = add(state[6], t6);
		state[7] = add(state[7], t7);
	}
